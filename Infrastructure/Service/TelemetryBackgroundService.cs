        ...
        public class TelemetryBackgroundService : BackgroundService
        {
            ...
            private async void OnReceived(object? sender, BasicDeliverEventArgs ea)
            {
                ...
                try
                {
                    ...
                    jsonString = jsonString.Trim('\0', '\r', '\n', ' ', '+');
                    //var jsonString = Encoding.UTF8.GetString(body);
                    ...
                    if (!_mappingCache.TryGet(dto.DeviceId, dto.deviceSlaveId,dto.RegisterAddress, out var mapping))
                    {
                        _logger.LogDebug("Telemetry unmapped for Device:{Device} Port:{Port}", dto.DeviceId, dto.deviceSlaveId,dto.RegisterAddress);
                        _channel.BasicAck(ea.DeliveryTag, false);
                        return;
                    }
                    ...
                    var influxDto = new InfluxTelementryDto
                    {
                        ...
                        deviceSlaveId = mapping.DeviceSlaveID,
                        SignalTypeId = mapping.SignalTypeId,
